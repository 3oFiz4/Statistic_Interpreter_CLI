    ...
    cfg.rules = [
        ...
        FormatRule(
            ...
            condition=lambda ctx: (
                ctx.numeric() == 0 and str(ctx.cell).strip() not in ("", "N/A")
            ),
        ...
        FormatRule(
            ...
            action=lambda ctx: (
                f"[bold bright_white]{ctx.display}[/bold bright_white]"
                if ctx.rank("VOLUME") >= 0.9
                else f"[white]{ctx.display}[/white]"
                if ctx.rank("VOLUME") >= 0.6
                else f"[#888888]{ctx.display}[/#888888]"
            ),
            target="col.VOLUME",
            priority=30,
        ),
        FormatRule(
            ...
            condition=lambda ctx: True,
            action=lambda ctx: (
                f"[bold #7b68ee]{ctx.display}[/bold #7b68ee]"
                if "Tech" in str(ctx.cell)
                else f"[bold #ff8c00]{ctx.display}[/bold #ff8c00]"
                if "Energy" in str(ctx.cell)
                else f"[bold #3cb371]{ctx.display}[/bold #3cb371]"
                if "Health" in str(ctx.cell)
                else f"[bold #cd853f]{ctx.display}[/bold #cd853f]"
                if "Finance" in str(ctx.cell)
                else ctx.display
            ),
            target="col.SECTOR",
            priority=35,
        ...
        FormatRule(
            ...
            condition=lambda ctx: True,
            action=lambda ctx: (
                f"[green]+{ctx.display}[/green]"
                if ctx.numeric("CHANGE") > 0
                else f"[red]{ctx.display}[/red]"
                if ctx.numeric("CHANGE") < 0
                else f"[dim]{ctx.display}[/dim]"
            ),
            target="row.CHANGE",
            priority=43,
        ),
        ...
        FormatRule(
            ...
            condition=lambda ctx: ctx.numeric("CHANGE") >= 10,
            action=lambda ctx: (
                f"[bold green on #001a00]{ctx.display}[/bold green on #001a00]"
            ),
            target="row.*",
            priority=52,
            stop_on_hit=True,
        ...
        FormatRule(
            ...
            condition=lambda ctx: (
                ctx.numeric("VOLUME") > 200_000 and ctx.numeric("CHANGE") > 5
            ),
            action=lambda ctx: (
        ...
        FormatRule(
            ...
            action=lambda ctx: (
                f"[bold cyan underline]{ctx.display}[/bold cyan underline]"
            ),
            target="row.TICKER",
            priority=60,
        ),
        FormatRule(
            ...
            action=lambda ctx: (
                f"[bold yellow italic]{ctx.display}[/bold yellow italic]"
            ),
            target="row.NOTES",
            priority=61,
        ),
        ...
        FormatRule(
            ...
            action=lambda ctx: (
                f"[bold bright_white underline]{ctx.display}[/bold bright_white underline]"
            ),
        ...
        FormatRule(
            ...
            condition=lambda ctx: ctx.matches(
                r"^(AAPL|MSFT|GOOG|AMZN|NVDA)$", "TICKER"
            ),
...
def make_populate_table(cfg: TableFormattingConfig):
    ...
    def _populate_table(self) -> None:
        ...
        for idx, item in enumerate(self.data):
            ...
            stripe = (
                cfg.stripe_even
                if display_idx % 2 == 0 and cfg.stripe_even
                else cfg.stripe_odd
                if display_idx % 2 != 0 and cfg.stripe_odd
                else ""
            )
